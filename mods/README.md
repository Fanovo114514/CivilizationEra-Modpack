# 文明纪元 - 模组下载清单

> ⚠️ **注意**：由于版权原因，本仓库不直接包含模组 jar 文件。请根据以下列表自行下载对应版本的模组，放入此文件夹（`mods/`）。

---

## 📥 下载方式推荐

**推荐使用 Modrinth App 或 Prism Launcher 自动下载**，也可以手动下载：

- **Modrinth**: https://modrinth.com/mods （大部分模组）
- **CurseForge**: https://www.curseforge.com/minecraft/mc-mods
- **MC百科**: https://www.mcmod.cn/ （中文介绍+下载链接）

---

## 📋 完整模组列表

### 🔧 基础框架层（13个）

| 序号 | 模组名称 | 英文名 | 版本要求 | 下载链接 | 说明 |
|------|----------|--------|----------|----------|------|
| 1 | FTB 任务 | FTB Quests | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/ftb-quests) | 任务系统核心，八纪元任务链 |
| 2 | FTB 库 | FTB Library | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/ftb-library-forge) | FTB 系列模组前置库 |
| 3 | FTB 团队架构 | FTB Team Architectury | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/ftb-architectury) | FTB 系列团队系统前置 |
| 4 | FTB任务UI美化 | FTB Quests UI Overhaul | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/ftb-quests-ui-overhaul) | 任务界面美化 |
| 5 | JEI | Just Enough Items | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/jei) | 配方查看器，配合时代锁使用 |
| 6 | Jade | Jade | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/jade) | 方块/实体信息HUD显示 |
| 7 | KubeJS | KubeJS | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/kubejs) | 配方脚本修改，实现时代锁 |
| 8 | 布料配置 | Cloth Config | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/cloth-config) | 配置界面库（多模组前置） |
| 9 | 小地图 | Xaero's Minimap | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/xaeros-minimap) | 小地图 |
| 10 | 世界地图 | Xaero's World Map | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/xaeros-world-map) | 世界地图 |
| 11 | 物品栏整理 | Inventory Profiles Next | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/inventory-profiles-next) | 物品栏管理、一键整理 |
| 12 | 按键管理 | Controlling | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/controlling) | 按键绑定管理 |
| 13 | 鼠标增强 | Mouse Tweaks | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/mouse-tweaks) | 鼠标物品操作增强 |

### ⚡ 优化层（4个）

| 序号 | 模组名称 | 英文名 | 版本要求 | 下载链接 | 说明 |
|------|----------|--------|----------|----------|------|
| 12 | Embeddium | Embeddium | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/embeddium) | 渲染性能优化（Sodium的Forge移植） |
| 13 | FerriteCore | FerriteCore | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/ferrite-core) | 内存占用优化 |
| 14 | ImmediatelyFast | ImmediatelyFast | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/immediatelyfast) | UI/物品渲染加速 |
| 15 | ModernFix | ModernFix | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/modernfix) | 综合性能优化 |

### ⚔️ 装备与武器系统（8个）

| 序号 | 模组名称 | 英文名 | 版本要求 | 下载链接 | 说明 | 时代解锁 |
|------|----------|--------|----------|----------|------|----------|
| 1 | 史诗战斗 | Epic Fight | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/epic-fight) | 动作战斗系统、技能、闪避、连击 | 炼造铁器 |
| 2 | 斯巴达武器 | Spartan Weaponry | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/spartan-weaponry) | 戟、锤、长柄刀、匕首等10+新武器 | 炼造铁器 |
| 3 | 钢铁护甲套装 | Iron's Spells n Spellbooks | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/irons-spells-n-spellbooks) | 魔法装备、法杖、法术书 | 发现电气 |
| 4 | 饰品栏 | Curios API | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/curios) | 扩展饰品栏（项链、戒指、护符等） | 刀耕火种 |
| 5 | 实用饰品 | Curious Elytra + Artifacts | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/artifacts) | 7种神器饰品，各有特殊能力 | 炼造铁器 |
| 6 | Tetra 模块化工具 | Tetra | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/tetra) | 模块化工具/武器，可自由改造部件 | 蒸汽革命 |
| 7 | 神秘武器 | Apotheosis | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/apotheosis) | 装备强化、附魔强化、Boss装备 | 发现电气 |
| 8 | 弓箭系统 | Archer's Paradox | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/archers-paradox) | 多种新弓、特殊箭矢、弓技能 | 刀耕火种 |

### 🏔️ 第1纪元：蛮荒（1个）

| 序号 | 模组名称 | 英文名 | 版本要求 | 下载链接 | 说明 |
|------|----------|--------|----------|----------|------|
| 16 | Terralith | Terralith | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/terralith) | 100+新生物群系，世界生成 |

### 🪓 第2纪元：原始（1个）

| 序号 | 模组名称 | 英文名 | 版本要求 | 下载链接 | 说明 |
|------|----------|--------|----------|----------|------|
| 17 | 初始工具包 | Starter Kit | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/starter-kit) | 控制玩家初始物品 |

### 🌾 第3纪元：农耕（6个）

| 序号 | 模组名称 | 英文名 | 版本要求 | 下载链接 | 说明 |
|------|----------|--------|----------|----------|------|
| 18 | 农夫乐事 | Farmer's Delight | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/farmers-delight) | 核心农业烹饪模组 |
| 19 | 农夫暇事 | Farmer's Respite | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/farmers-respite) | 茶与咖啡（FD附属） |
| 20 | Delightful | Delightful | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/delightful) | 更多食物作物（FD附属） |
| 21 | 蔬菜乐事 | Veggies Delight | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/veggies-delight) | 蔬菜扩展（FD附属） |
| 22 | 静谧四季 | Serene Seasons | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/serene-seasons) | 四季系统，影响农作物 |
| 23 | 悬赏任务 | Bountiful | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/bountiful) | 村庄悬赏板任务 |

### ⚔️ 第4纪元：铁器（5个）

| 序号 | 模组名称 | 英文名 | 版本要求 | 下载链接 | 说明 |
|------|----------|--------|----------|----------|------|
| 24 | 更多村民 | VillagersPlus | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/villagersplus) | 新村民职业 |
| 25 | 动态交易 | Dynamic Villager Trades | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/dynamic-villager-trades) | 村民交易动态变化 |
| 26 | 结构变体 | Repurposed Structures (Forge) | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/repurposed-structures-forge) | 原版结构变体 |
| 27 | 传送石 | Waystones | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/waystones) | 传送石/传送点 |
| 28 | Mowzie的怪物 | Mowzie's Mobs | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/mowzies-mobs) | 精英怪物/Boss |

### ⚙️ 第5纪元：蒸汽（7个）

| 序号 | 模组名称 | 英文名 | 版本要求 | 下载链接 | 说明 |
|------|----------|--------|----------|----------|------|
| 29 | 机械动力 | Create | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/create) | 齿轮/传送带/物理传动 |
| 30 | 蒸汽与铁路 | Create: Steam 'n' Rails | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/create-steam-n-rails) | 蒸汽机车/铁路（Create附属） |
| 31 | 沉浸工程 | Immersive Engineering | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/immersive-engineering) | 大型多方块/输电线杆 |
| 32 | 多级熔炉 | Iron Furnaces | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/iron-furnaces) | 铁/金/钻石等多级熔炉 |
| 33 | 高效管道 | Pipez | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/pipez) | 高效管道（物品/流体/能量） |
| 34 | 更好的地牢 | YUNG's Better Dungeons | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/yungs-better-dungeons) | 地牢重新设计 |
| 35 | 更好的要塞 | YUNG's Better Strongholds | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/yungs-better-strongholds) | 要塞改造 |

### ⚡ 第6纪元：电气（9个）

| 序号 | 模组名称 | 英文名 | 版本要求 | 下载链接 | 说明 |
|------|----------|--------|----------|----------|------|
| 36 | 通用机械 | Mekanism | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/mekanism) | 矿物五倍处理/数字矿工/物流 |
| 37 | 通用机械-发电机 | Mekanism Generators | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/mekanism-generators) | Mek能源扩展 |
| 38 | 通用机械-工具 | Mekanism Tools | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/mekanism-tools) | Mek工具扩展 |
| 39 | 通用机械-附加 | Mekanism Additions | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/mekanism-additions) | Mek附加内容 |
| 40 | 热力膨胀 | Thermal Expansion | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/thermal-expansion) | RF能源/各种机器 |
| 41 | 热力物流 | Thermal Dynamics | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/thermal-dynamics) | RF/流体/物品管道 |
| 42 | 应用能源2 | Applied Energistics 2 | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/ae2) | 数字化存储/自动合成 |
| 43 | 抽屉存储 | Storage Drawers | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/storage-drawers) | 大容量同种物品存储 |
| 44 | 潜影盒预览 | Shulker Box Tooltip | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/shulker-box-tooltip) | 潜影盒内容预览 |

### 💻 第7纪元：信息（4个）

| 序号 | 模组名称 | 英文名 | 版本要求 | 下载链接 | 说明 |
|------|----------|--------|----------|----------|------|
| 45 | 物流管道2 | Logistic Pipes 2 | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/logistic-pipes-2) | 智能物品路由 |
| 46 | 史诗战斗 | Epic Fight | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/epic-fight) | 战斗系统增强 |
| 47 | 变异怪物 | Mutant Monsters | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/mutant-monsters) | 变异Boss |
| 48 | 亚历克斯的生物 | Alex's Mobs | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/alexs-mobs) | 85+新生物 |

### 🚀 第8纪元：星际（4个）

| 序号 | 模组名称 | 英文名 | 版本要求 | 下载链接 | 说明 |
|------|----------|--------|----------|----------|------|
| 49 | 遥远星际 | Ad Astra | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/ad-astra) | 火箭/太空服/月球/火星 |
| 50 | 更好的末地 | BetterEnd Forge | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/betterend-forge) | 末地增强（需BCLib+WunderLib前置） |
| 51 | 更好的下界要塞 | YUNG's Better Nether Fortresses | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/yungs-better-nether-fortresses) | 下界要塞改造 |
| 52 | 下界扩展 | Infernal Expansion Redux | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/infernal-expansion-redux) | 下界生物增强 |

### 🌍 世界生成与氛围（5个）

| 序号 | 模组名称 | 英文名 | 版本要求 | 下载链接 | 说明 |
|------|----------|--------|----------|----------|------|
| 53 | 地下结构 | Geophilic | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/geophilic) | 地下结构增强 |
| 54 | 地壳生成 | Tectonic | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/tectonic) | 地壳生成增强 |
| 55 | 脚步声 | Presence Footsteps | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/presence-footsteps) | 脚步声增强 |
| 56 | 声音物理 | Sound Physics Remastered | 1.20.1-最新 | [Modrinth](https://modrinth.com/mod/sound-physics-remastered) | 声音物理模拟 |
| 57 | 环境音效 | Ambient Sounds Mod | 1.20.1-最新 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/ambient-sounds-mod) | 环境音效增强（可选，若链接失效可在CurseForge搜索"Ambient Sounds"） |

### 🏛️ 经济/势力辅助（2个）

> ⚠️ **注意**：经济和势力系统已集成到自创模组「势力与经济」中，无需额外下载第三方模组。
> 以下模组仅作参考，如需额外功能可选择安装：

| 序号 | 模组名称 | 英文名 | 版本要求 | 下载链接 | 说明 |
|------|----------|--------|----------|----------|------|
| 58 | 经济系统（可选） | 已集成到自创模组 | - | - | 货币系统、ATM、商店等功能已在「势力与经济」模组中实现 |
| 59 | 势力系统（可选） | 已集成到自创模组 | - | - | 六大势力、声望、外交等功能已在「势力与经济」模组中实现 |

### 🔧 前置模组（重要！）

以下是部分模组需要的前置库，请一并下载：

| 前置模组 | 被哪些模组依赖 | 下载链接 |
|----------|----------------|----------|
| FTB Library | FTB Quests | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/ftb-library-forge) |
| FTB Team Architectury | FTB Quests | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/ftb-architectury) |
| BCLib (NeoForge版) | BetterEnd Forge | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/bclib-neoforge) |
| WunderLib / WorldWeaver | BetterEnd Forge | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/worldweaver-neoforge) |
| GeckoLib | Alex's Mobs, Mowzie's Mobs | [Modrinth](https://modrinth.com/mod/geckolib) |
| Puzzles Lib | Mutant Monsters | [Modrinth](https://modrinth.com/mod/puzzles-lib) |
| Bookshelf | 部分模组 | [Modrinth](https://modrinth.com/mod/bookshelf-lib) |
| Architectury API | 部分模组 | [Modrinth](https://modrinth.com/mod/architectury-api) |
| Patchouli | 部分模组（指南书） | [Modrinth](https://modrinth.com/mod/patchouli) |
| Curios API | 部分模组（饰品栏） | [Modrinth](https://modrinth.com/mod/curios) |
| Cloth Config | 多数模组配置界面 | [Modrinth](https://modrinth.com/mod/cloth-config) |
| Kotlin for Forge | Logistic Pipes 2 | [CurseForge](https://www.curseforge.com/minecraft/mc-mods/kotlin-for-forge) |

---

## 🔨 自创模组（4个）

以下是本整合包的自创模组，位于仓库源码中，需要自行编译：

| 模组名称 | 英文名 | 说明 | 源码路径 |
|----------|--------|------|----------|
| 文明核心 | Civilization Core | 时代锁、进化碎片、货币系统 | [civilization-core/](../civilization-core/) |
| 势力与经济 | Faction & Economy | 六大势力、声望系统、市场经济 | [faction-economy/](../faction-economy/) |
| 科技研究 | Tech Research | 科技碎片、蓝图系统、研究台 | [tech-research/](../tech-research/) |
| 蛮荒开局 | Barbarian Start | 蛮荒限制、生火机制、陷阱 | [barbarian-start/](../barbarian-start/) |

**编译方法**：
```bash
./gradlew build
# 编译产物在各模块的 build/libs/ 目录下
```

---

## 📦 快速安装指南

### 方法一：使用启动器一键导入（推荐）
1. 下载 `CivilizationEra-Modpack-1.0.0.zip`
2. 打开 CurseForge / Prism Launcher / HMCL
3. 选择「导入整合包」，选择下载的 zip 文件
4. 启动器会自动下载所有模组

### 方法二：手动安装
1. 安装 Forge 1.20.1-47.3.0
2. 下载上表中所有模组 jar 文件
3. 放入 `.minecraft/mods/` 文件夹
4. 把 `config/`、`kubejs/`、`defaultconfigs/` 覆盖到对应目录
5. 把 `resourcepacks/` 中的汉化包放入资源包文件夹

---

## ⚠️ 注意事项

1. **版本对应**：请确保所有模组都是 **1.20.1 Forge** 版本
2. **前置模组**：不要忘记下载上表中的前置模组
3. **模组顺序**：一般不需要手动排序，启动器会自动处理
4. **内存分配**：建议分配 **4-6GB** 内存
5. **Java版本**：需要 **Java 17**

---

## 📝 更新日志

- v1.0.1 - 修复失效链接：
  - FTB Quests 改用 CurseForge 链接（Modrinth 上不存在）
  - 添加 FTB Library 和 FTB Team Architectury 前置
  - 修正 BCLib 和 WunderLib 为 NeoForge/Forge 版本
  - 经济/势力系统改为集成到自创模组
  - 补充更多前置模组说明
- v1.0.0 - 初始版本，包含 57 个第三方模组 + 4 个自创模组
